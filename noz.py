...
class NozMenu:
    ...
    def __init__(self):
        ...
        self.noz_wall = 'https://www.facebook.com/Restauracjanoz'
    ...
    def extract_lunch_menu(self, post):
        date = post.split('·')[0].split('NÕŻ')[-1]
        menu = post.split('·')[1].split('Wszystkie reakcje')[0].replace('\n', '')
        if 'godz' in date:
            if str(date.split('godz')[0]) < 12:
                todays_menu = menu
        else:
